...
func (pk *BiomeDefinitionList) Marshal(io protocol.IO) {
	if proto.IsProtoGTE(io, proto.ID800) {
		protocol.Slice(io, &pk.BiomeDefinitions)
		protocol.FuncSlice(io, &pk.StringList, io.String)
	} else {
		io.Bytes(lo.ToPtr(append([]byte(nil), rawOldSerializedBiomeDefinitionList...)))
	}
}

var rawOldSerializedBiomeDefinitionList []byte
...
func init() {
	...
	rawOldSerializedBiomeDefinitionList, _ = base64.StdEncoding.DecodeString(s)
}
